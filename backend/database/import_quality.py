"""Lê a planilha de qualidade e gera o SQL que popula o banco infinity_metalique.

O projeto não usa Composer, e ler .xlsm em PHP puro exigiria escrever um parser de
OOXML. Por isso a importação fica em Python com openpyxl e produz um arquivo .sql
revisável, importado do mesmo jeito que schema.sql.
    ...
    C:\\xampp\\mysql\\bin\\mysql.exe -u root --default-character-set=utf8mb4 \\
        infinity_metalique < backend/database/seed_quality.sql
...
def build(path: Path) -> tuple[list[str], dict[str, int]]:
    ...
    # --- Montagem do arquivo ---------------------------------------------------------
    statements.append("-- Gerado por backend/database/import_quality.py. Não editar à mão.")
